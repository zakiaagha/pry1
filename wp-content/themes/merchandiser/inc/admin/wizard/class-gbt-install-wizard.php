...
if ( ! class_exists( 'GBT_InstallWizard' ) ) {
	...
	class GBT_InstallWizard {
		...
		public function setupWizard() {
			...
			$default_steps = array(
				'introduction' => array(
					'name'    => __( 'Introduction', 'merchandiser' ),
		...
		public function setup_wizard_steps() {
			...
			<div class="gbt-wizard-logo <?php echo GBTHELPERS::theme_slug(); ?>">
				<a href="#">
					<img src="<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/merchandiser-logo.png'; ?>" <?php echo file_exists(get_template_directory() .'/inc/admin/wizard/images/merchandiser-logo@2x.png')? 'srcset="'.get_template_directory_uri() .'/inc/admin/wizard/images/merchandiser-logo@2x.png 2x'.'"': '' ; ?> alt="Logo">
				</a>
			</div>
		...
		public function gbt_setup_introduction() {
			?>
				<div class="wrapper wizard-introduction" style="background-image:url(<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/merchandiser-setup.jpg'; ?>);">
					<div class="center">
						<h1><?php esc_html_e( GBTHELPERS::theme_name() . '\'s', 'merchandiser');?> <br/> <?php esc_html_e('Theme Setup Wizard', 'merchandiser' ); ?></h1>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" 
							class="button-primary button button-large button-next">
							<?php esc_html_e( 'Get Started', 'woocommerce' ); ?>
						</a>
						<p><?php esc_html_e( 'The quickest way to setup the theme', 'merchandiser'); ?> <br /> <?php esc_html_e('and start working on your site.', 'merchandiser' );?></p>
					</div>
				</div>
			...
			?>
				<div class="wrapper wizard-plugins">
					...
					<div class="content-info">
						<h1><?php esc_html_e( 'Plugin Installation', 'merchandiser' ); ?></h1>
						<p><?php _e( 'Install the required plugins before importing the demo content.', 'merchandiser' ); ?></p>
					</div>
					<div class="plugins">
					<?php
						// global $GBT_Plugins;
					...
					foreach ( $plugins as $slug => $plugin ) {
						if ( isset($plugin['demo_required']) && $plugin['demo_required'] === true ) : ?>
							<div class="plugin <?php echo esc_attr( $slug ); ?>">
								<p class="required-tag"><?php esc_html_e( 'Required', 'merchandiser' ); ?></p>
								...
								<div class="plugin-install">
									...
									<!-- <span class="plugin-description"><?php echo isset( $plugin['description'] )? esc_html_e( $plugin['description'], 'merchandiser' ) : '';?></span> -->
									...
									<?php if ( ! empty( $plugin['status'] ) && ($plugin['status'] != 'no-action') ) : ?>
											<a  class="button ajax-request <?php echo esc_html( $plugin['status'] ); ?>-now" 
												...
												<?php echo ($plugin['status'] == 'install') ? esc_html_e( 'Install Now', 'merchandiser' ) : ''; ?>
												<?php echo ($plugin['status'] == 'update') ? esc_html_e( 'Update Now', 'merchandiser' ) : ''; ?>
												<?php echo ($plugin['status'] == 'activate') ? esc_html_e( 'Activate', 'merchandiser' ) : ''; ?>
											</a>

										<?php else : ?>
											<a class="button button-disabled">
												<?php  esc_html_e( 'Active', 'merchandiser' ); ?>
											</a>
										<?php endif; ?>
									</div>
								</div>
					...
					<div class="buttons">
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button skip"><?php esc_html_e( 'Skip', 'merchandiser' ); ?></a>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button install "><?php esc_html_e( 'Install', 'merchandiser' ); ?></a>
					</div>
				</div>
		...
		public function gbt_setup_demo() {
			...
			<div class="wrapper wizard-demo-import">
				...
				<div class="content-info">
					<h1><?php esc_html_e( 'Demo Content Import', 'merchandiser' ); ?></h1>
					<p><?php esc_html_e( 'Start with pre-built page layouts, dummy product pages,', 'merchandiser');?><br/> <?php esc_html_e('blog posts and widgets.', 'merchandiser' ); ?></p>
				</div>
				...
				<div class="demo-icon <?php echo ( ! $is_required_plugins === true) ? 'error' : '' ?>">
					...
					<?php else : ?>
						<p class="error-info"><?php _e( 'Please make sure <b>Visual Composer</b> and <b>WooCommerce</b> are installed and activated before importing the demo content.', 'merchandiser' ); ?></p>
					<?php endif; ?>
				</div>
				...
				<div class="buttons">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button skip"><?php esc_html_e( 'Skip', 'merchandiser' ); ?></a>
					<?php if ( $is_required_plugins === true ) : ?>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button install"><?php esc_html_e( 'Continue', 'merchandiser' ); ?></a>
					<?php else : ?>
						<a href="<?php echo esc_url( $this->get_prev_step_link() ); ?>" class="button "><?php esc_html_e( 'Go back', 'merchandiser' ); ?></a>
					<?php endif; ?>
				</div>

				<!-- <p class="ocdi__ajax-loader  js-ocdi-ajax-loader">
					<span class="spinner"></span> <?php esc_html_e( 'Importing, please wait!', 'merchandiser' ); ?>
				</p> -->
		...
		public function gbt_setup_final() {
			...
			<div class="wrapper wizard-ready">
				<div class="content-done" style="background-image:url(<?php echo get_template_directory_uri() .'/inc/admin/wizard/images/merchandiser-setup.jpg'; ?>);">
					<div class="center">
						<h1><?php _e( 'Setup has been<br/> completed successfully!', 'merchandiser' ); ?></h1>
						<a href="<?php echo esc_url( site_url() ); ?>" class="button button-primary"><?php esc_html_e( 'View Site', 'merchandiser' );?></a>
						<p><?php esc_html_e('You should be able to start working on your site now.','merchandiser');?> <br/>
						<?php esc_html_e('Best of luck with your project!', 'merchandiser'); ?></p>
					</div>
				</div>
				
				<div class="further-info">
					<h2><?php esc_html_e( 'What\'s next?', 'merchandiser' );?></h2>
					<div class="column">
						...
						<ul>
							<li>
								<a href="<?php echo esc_url( admin_url( 'customize.php' ) ); ?>">
									...
									<?php esc_html_e( 'Customize the theme', 'merchandiser' ); ?>
								</a>
							</li>
							<li>
								<a href="<?php echo esc_url( admin_url( 'index.php?page=wc-setup' ) );?>">
									<span class="dashicons dashicons-admin-settings"></span>
									<?php esc_html_e( 'WooCommerce Setup Wizard', 'merchandiser' ); ?>
								</a>
							</li>
						</ul>
					</div>
						...
						<ul>
							<li>
								...
								<a href="<?php print $activate; ?>">
									<span class="dashicons dashicons-update"></span><?php esc_html_e('Activate Theme Updates', 'merchandiser' ); ?>
								</a>
							</li>
							<li><a href="<?php echo esc_url( GBTHELPERS::support_link() ); ?>" target="_blank"><span class="dashicons dashicons-editor-help"></span><?php esc_html_e( 'Help Center / Support', 'merchandiser' ); ?></a></li>
						</ul>
					</div>
				</div>
